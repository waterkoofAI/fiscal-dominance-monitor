...
def _key_metrics(f: dict, p: Panel, when: date, spot: dict | None) -> list[dict]:
    ...
    out = [
        ...
        row("nfp", "非农月增", "PAYEMS", f.get("payems_chg_1m"), "",
            f.get("payems_avg3m"), "", F + "PAYEMS"),
        row("unrate", "失业率", "UNRATE", f.get("unrate_level"), "%",
            f.get("unrate_chg_3m"), "pp", F + "UNRATE"),
        row("claims", "初请失业金", "ICSA", f.get("icsa_level"), "",
            f.get("icsa_chg_4w"), "", F + "ICSA"),
